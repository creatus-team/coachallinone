...
export async function POST(request: Request) {
    try {
        ...
        if (!name || !phone) {
            console.error('[Webhook] Missing name or phone');
            return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
        }

        // 2. Determine if NEW or RENEWAL
        ...
        if (isRenewal) {
            ...
            if (!parsed) {
                // Cannot parse -> Notify admin
                await notifyAdmin(`파싱 실패: ${name} (${phone}) - ${purchaseRaw}`);
                return NextResponse.json({ success: true, warning: 'Could not parse, admin notified' });
            }
            return await handleNewEnrollment(name, phone, parsed);
        }
    } catch (error: any) {
...
async function handleNewEnrollment(
    ...
    `, [name, phone, info.productType]);
        return NextResponse.json({ success: true, warning: 'Coach not found, admin notified' });
    }
  ...
  `, [coach.id, info.dayOfWeek, info.startTime]);
    ...
    if (slotRes.rows.length === 0) {
      ...
      ON CONFLICT (phone) DO UPDATE SET name = $1, status = 'pending', product_type = $3
    `, [name, phone, info.productType]);
        return NextResponse.json({ success: true, warning: 'Slot not available, admin notified' });
    }
...
async function handleRenewal(name: string, phone: string) {
    ...
    if (userRes.rows.length === 0) {
        ...
        return NextResponse.json({ success: true, warning: 'User not found for renewal, admin notified' });
    }
  ...
  `, [user.id]);
    ...
    if (sessionRes.rows.length === 0) {
        ...
        return NextResponse.json({ success: true, warning: 'No previous session found, admin notified' });
    }
...
async function notifyAdmin(message: string) {
    ...
    if (!adminPhone) {
        console.warn('[Admin] No admin phone configured');
        return;
    }
    await sendSMS({
        to: adminPhone,
