...
export async function POST(request: Request) {
    try {
        ...
        console.log('[코칭신청서 Webhook] Received');
        ...
        for (const field of body.data.fields) {
            ...
            if (field.label.includes('주차') || field.label.includes('week')) {
                weekNumber = String(field.value || '');
            }
        }

        if (!phone) {
            console.log('[코칭신청서] 전화번호 없음, 스킵');
    ...
    `, [phone]);
        ...
        if (sessionRes.rows.length === 0) {
            console.log(`[코칭신청서] 해당 전화번호(${phone})의 수강생/코치 정보 없음`);
            await query(`
        INSERT INTO message_logs (type, recipient_name, recipient_phone, content, status)
        VALUES ('COACHING_FORM', $1, $2, '담당 코치 없음 - 알림 미발송', 'SKIPPED')
      `, [studentName || 'Unknown', phone]);
        ...
        const weekInfo = weekNumber ? `(${weekNumber})` : '';

        // 3. Send SMS to Student (Receipt Confirmation)
        await sendSMS({
            to: phone,
            text: `[크리투스] ${user_name || studentName}님, 코칭신청서가 정상적으로 접수되었습니다. (${timestamp})`,
            type: 'COACHING_FORM',
        ...
        await sendSMS({
            to: coach_phone,
            text: `[크리투스] ${coach_name} 코치님, ${user_name || studentName}님의 코칭신청서${weekInfo}가 제출되었습니다.\n확인해주세요!\n\n[피드백 신청 DB]\n${NOTION_DB_LINK}`,
            type: 'COACHING_FORM',
            recipientName: coach_name,
        });

        console.log(`[코칭신청서] ${user_name} → ${coach_name} 알림 발송 완료`);
        return NextResponse.json({ success: true, notified: coach_name });

    } catch (error: any) {
        console.error('[코칭신청서 Webhook Error]', error);
